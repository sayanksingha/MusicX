...
export default function App() {
  ...
  // Live suggestions: keep typing completely local/responsive and only ask the
  // server after the user pauses. The previous implementation depended on
  // large result arrays, causing the effect to restart on every search result.
  const suggestionCacheRef = React.useRef<Map<string, Song[]>>(new Map());
  const suggestionRequestRef = React.useRef<AbortController | null>(null);
  const suggestionSourceRef = React.useRef<Song[]>([]);

  useEffect(() => {
    suggestionSourceRef.current = Array.from(new Map(
      [...trendingSongs, ...likedSongs, ...historySongs, ...searchResults].map((song) => [song.id, song])
    ).values());
  }, [trendingSongs, likedSongs, historySongs, searchResults]);
  ...
  useEffect(() => {
    ...
    const localMatches = suggestionSourceRef.current
      .filter((song) => `${song.title} ${song.artist}`.toLowerCase().includes(key))
      .slice(0, 6);

    const cached = suggestionCacheRef.current.get(key);
    if (cached) {
      setSearchSuggestions(cached);
      return;
    }

    // Local matches are shown immediately; network suggestions never block typing.
    ...
    const timer = window.setTimeout(async () => {
      try {
        ...
        if (!controller.signal.aborted) {
          suggestionCacheRef.current.set(key, songs);
          setSearchSuggestions(songs.length ? songs : localMatches);
        }
      } catch (e: any) {
        if (e?.name !== 'AbortError') console.debug('Suggestion request failed');
      }
    }, 650);
  ...
  const handleSearchSubmit = async (query: string) => {
    ...
    try {
      ...
      const data = await res.json();
      startTransition(() => setSearchResults(data.songs || []));
    } catch (e) {
